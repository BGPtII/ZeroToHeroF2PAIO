...
public class SmartMouseMultiDir implements MouseAlgorithm {
    ...
    public boolean handleMovement(AbstractMouseDestination destination) {
        ...
        if (isOutsideCanvas(current) && isOutsideCanvas(target)) {
        ...
        for (int i = 0; i < path.size(); i++) {
            ...
            moveSmoothlyWithRecordedTiming(Mouse.getPosition(), stepDurationSeconds);
        }
    ...
    private boolean isOutsideCanvas(Point point) {
        return point.getX() < 0
                || point.getY() < 0
                || point.getX() >= SCREEN_WIDTH
                || point.getY() >= SCREEN_HEIGHT;
    }
     ...
     */
    private void moveSmoothlyWithRecordedTiming(Point end, double totalMovementSeconds) {
    ...
    private List<List<Double>> getPathOffsets(double distance, String direction) {
        ...
        int index = ScriptData.SECURE_RANDOM.nextInt(directionPaths.size());
    ...
    private double randomDouble(double min, double max) {
        return min + (max - min) * ScriptData.SECURE_RANDOM.nextDouble();
    }
    ...
    private void loadMouseData() {
        Gson gson = new Gson();
        try (InputStream is = getClass().getResourceAsStream("/data/MouseData.json")) {
            if (is == null) {
