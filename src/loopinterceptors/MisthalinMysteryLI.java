 ...
 * - Manor area contained in one region, therefore use local obstacles opposed to web node addition
 */
public class MisthalinMysteryLI extends LoopInterceptor {
    ...
    private boolean orientMirror(int x, int y) { // x, y = translation values
        ...
        if (playerTile.equals(mirrorTileTranslated)) {
            if (ScriptData.currentNPC.interact()) {
                Sleep.sleepUntil(() -> Players.getLocal().getAnimation() != -1, ScriptData.SECURE_RANDOM.nextInt(5000 - 1000 + 1) + 1000);
                Sleep.sleepUntil(() -> Players.getLocal().getAnimation() == -1, ScriptData.SECURE_RANDOM.nextInt(10000 - 5000 + 1) + 5000, 300);
                return true;
            }
        }
        else if (Walking.walkExact(mirrorTileTranslated)) {
            Sleep.sleepUntil(() -> Players.getLocal().getTile().equals(mirrorTileTranslated), ScriptData.SECURE_RANDOM.nextInt(10000 - 5000 + 1) + 5000, 300);
            return true;
        }
        return false;
    }
        ...
        switch (PlayerSettings.getBitValue(3468)) {
                ...
                if (STARTING_AREA.contains(Players.getLocal())) {
                    if ((ScriptData.currentNPC = NPCs.closest(npc -> npc.isInteracting(Players.getLocal()) && npc.getName().equals("Giant rat") && npc.getAnimation() != -1)) != null) {
                        if (ScriptData.currentNPC.interact("Attack")) {
                            Sleep.sleepUntil(() -> ScriptData.currentNPC == null || !ScriptData.currentNPC.exists(), ScriptData.SECURE_RANDOM.nextInt(240000 - 120000 + 1) + 120000, 300);
                        }
                    }
                ...
                if (Inventory.contains(21052)) { // Manor key
                    ...
                    if (ScriptData.currentGameObject.getName().startsWith("A")) { // A barrel of rainwater#Full
                        if (Inventory.isItemSelected()) {
                            ...
                            Sleep.sleepUntil(Inventory::isItemSelected, ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 300);
                        }
                    }
                    else if (ScriptData.currentGameObject.interact("Search")) {
                        Sleep.sleepUntil(() -> Inventory.contains(21052), ScriptData.SECURE_RANDOM.nextInt(10000 - 3000 + 1) + 3000, 300);
                    }
                }
                break;
            case 30:
            case 35:
                if (Inventory.contains(21056)) { // Notes
                    if (Inventory.interact(21056, "Read")) {
                        Sleep.sleepUntil(() -> Widgets.isVisible(70, 3), ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 300);
                    }
                }
                ...
                else if ((ScriptData.currentGameObject = GameObjects.closest(gameObject -> gameObject.getId() == 2266 && gameObject.hasAction("Take"))) != null) { // Notes
                    Logger.log("currentGameObject: " + ScriptData.currentGameObject);
                    Logger.log("x, y, z: " + ScriptData.currentGameObject.getX() + ", " + ScriptData.currentGameObject.getY() + ", " + ScriptData.currentGameObject.getZ());
                    Logger.log("hasAction Take: " + ScriptData.currentGameObject.hasAction("Take"));
            ...
            case 40:
                if (Widgets.isVisible(70, 104)) {
                    if ((ScriptData.currentWidgetChild = Widgets.get(70, 104)) != null && ScriptData.currentWidgetChild.interact("Close")) {
                        Sleep.sleepUntil(() -> !Widgets.isVisible(70, 104), ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 300);
                    }
                }
            ...
            case 45:
                if (Inventory.contains(21053)) { // Ruby key
                    ...
                    if (ScriptData.currentGameObject.interact()) {
                        Sleep.sleepUntil(Dialogues::inDialogue, ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 300);
                    }
                }
                else {
                    Inventory.use(946);
            ...
            case 50:
                if (!DEMOLITION_CANDLES.contains(Players.getLocal())) {
                    ...
                    if (ScriptData.currentGameObject == null || !ScriptData.currentGameObject.exists() || ScriptData.currentGameObject.getId() != 30146) { // Shelves (containing Tinderbox)
                        ...
                        Sleep.sleepUntil(() -> Inventory.contains(590), ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 300);
                    }
                }
                else {
                        ...
                        case 1: // SW Candle
                            if ((ScriptData.currentGameObject = GameObjects.closest(29655)) != null) {
                                if (ScriptData.currentGameObject.getName().equals("Candle")) {
                                    ...
                                    if (ScriptData.currentGameObject.interact()) {
                                        Sleep.sleepUntil(ScriptData.IN_DIALOGUE, ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 200);
                                    }
                                }
                                else {
                                    Inventory.use(590);
            ...
            case 70:
                if (Inventory.contains(21057)) {
                    if (Inventory.interact(21057, "Read")) {
                        Sleep.sleepUntil(() -> Widgets.isVisible(70, 3), ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 200);
                    }
                }
                    ...
                    if ((ScriptData.currentGameObject = GameObjects.closest("Damaged wall")) != null) {
                        if (ScriptData.currentGameObject.interact("Climb")) {
                            Sleep.sleepUntil(() -> !DEMOLITION_CANDLES.contains(Players.getLocal()), ScriptData.SECURE_RANDOM.nextInt(15000 - 5000 + 1) + 5000, 300);
                        }
                    }
                }
                ...
                if (Widgets.isVisible(70, 104)) {
                    ...
                    if ((pianoAttempts == 0 || pianoAttempts == 3) && Widgets.isVisible(554, 21)) { // 2nd key (D)
                        if ((ScriptData.currentWidgetChild = Widgets.get(554, 21)) != null && ScriptData.currentWidgetChild.interact()) {
                            Sleep.sleepUntil(() -> pianoAttempts != PlayerSettings.getBitValue(4049), ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 200);
                        }
                    }
                        ...
                        if (ScriptData.currentGameObject.hasAction("Search")) {
                            ...
                            Sleep.sleepUntil(() -> Widgets.isVisible(554, 21), ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 200);
                        }
                    }
                }
                        ...
                        switch (attempts) {
                            case 0: // Sapphire
                                if ((ScriptData.currentWidgetChild = Widgets.get(555, 19)) != null && ScriptData.currentWidgetChild.interact()) {
                                    Sleep.sleepUntil(() -> PlayerSettings.getBitValue(4050) != attempts, ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 200);
                                }
                                break;
                            case 1: //Diamond
                ...
                if (BOSS.contains(Region.fromInstance(Players.getLocal().getTile()))) {
                    if (Equipment.contains(21059)) { // Killer's knife
                        ...
                        if (Inventory.interact(21059, "Wield")) {
                            Sleep.sleepUntil(() -> Equipment.contains(21059), ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 200);
                        }
                    }
                        ...
                        if (ScriptData.currentGroundItem.interact("Take")) {
                            Sleep.sleepUntil(() -> Inventory.contains("Killer's knife"), ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 200);
                        }
                    }
                }
                else if (!ScriptData.walkToArea(BOSS)) {
            ...
            case 125:
                if ((ScriptData.currentGameObject = GameObjects.closest("Door")) != null) {
                    if (ScriptData.currentGameObject.interact("Open")) {
                        Sleep.sleepUntil(() -> NPCs.closest("Hewey") == null, ScriptData.SECURE_RANDOM.nextInt(20000 - 5000 + 1) + 5000, 200);
                    }
                }
                break;
            case 130:
