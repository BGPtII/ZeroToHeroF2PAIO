...
/**
 * - Logs worth the most gp (of Willow, Oak, normal) as of 2025-11-14
 */
public class ChoppingLogs implements Node {
    ...
    public int loop() {
        ...
        if (ScriptData.currentGameObject == null || !ScriptData.currentGameObject.exists()) {
            List<GameObject> opts = GameObjects.all(gameObject -> ScriptData.currentArea.contains(gameObject)
                    && gameObject.hasAction("Chop down") && gameObject.distance() <= ScriptData.currentEntityDistance && (gameObject.getName().equals(ScriptData.currentEntityName) || (ScriptData.currentEntityName.startsWith("T") && (gameObject.getName().equals("Dead tree") || gameObject.getName().equals("Evergreen tree")))));
            if (opts.isEmpty()) {
